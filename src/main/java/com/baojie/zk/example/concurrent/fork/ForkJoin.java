...
public class ForkJoin extends AbstractExecutorService {
    ...
    public static interface ForkJoinWorkerThreadFactory {
         ...
         */
        public ForkJoinWorkerThread newThread(ForkJoin pool);
    }
    ...
    static final class DefaultForkJoinWorkerThreadFactory implements ForkJoinWorkerThreadFactory {
        public final ForkJoinWorkerThread newThread(ForkJoin pool) {
    ...
    static final class WorkQueue {
        ...
        final ForkJoin pool;   // the containing pool (may be null)
        final ForkJoinWorkerThread owner; // owning thread or null if shared
        volatile Thread parker;    // == owner during call to park; else null
        volatile ForkJoinTask<?> currentJoin;  // task being joined in awaitJoin
        volatile ForkJoinTask<?> currentSteal; // mainly used by helpStealer

        WorkQueue(ForkJoin pool, ForkJoinWorkerThread owner) {
            this.pool = pool;
            this.owner = owner;
         ...
         */
        final void transferStealCount(ForkJoin p) {
            AtomicLong sc;
    ...
    private int awaitRunStateLock() {
        ...
        for (int spins = SPINS, r = 0, rs, ns; ; ) {
            if (((rs = runState) & RSLOCK) == 0) {
                ...
                r = LocalRandom.nextSecondarySeed();
            } else if (spins > 0) {
                r ^= r << 6;
                r ^= r >>> 21;
                r ^= r << 7; // xorshift
    ...
    private WorkQueue findNonEmptyStealQueue() {
        ...
        int r = LocalRandom.nextSecondarySeed();
        if ((ws = workQueues) != null && (m = ws.length - 1) >= 0) {
            for (int origin = r & m, k = origin, oldSum = 0, checkSum = 0; ; ) {
                WorkQueue q;
                int b;
    ...
    private void externalSubmit(ForkJoinTask<?> task) {
        ...
        if ((r = LocalRandom.getProbe()) == 0) {
            LocalRandom.localInit();
            r = LocalRandom.getProbe();
        }
        for (; ; ) {
            WorkQueue[] ws;
            WorkQueue q;
     ...
     *                                  java.lang.RuntimePermission}{@code ("modifyThread")}
     */
    public ForkJoin(int parallelism) {
     ...
     * makeCommonPool.
     */
    private ForkJoin(int parallelism,
            ForkJoinWorkerThreadFactory factory,
            Thread.UncaughtExceptionHandler handler,
            int mode,
     ...
     * @since 1.8
     */
    public static ForkJoin commonPool() {
     ...
     * thread invoking {@link ForkJoin#managedBlock(ManagedBlocker)}.
    ...
    static {
        ...
        common = java.security.AccessController.doPrivileged
                (new java.security.PrivilegedAction<ForkJoin>() {
                    public ForkJoin run() {
     ...
     */
    private static ForkJoin makeCommonPool() {
        int parallelism = -1;
        ...
        return new ForkJoin(parallelism, factory, handler, LIFO_QUEUE,
