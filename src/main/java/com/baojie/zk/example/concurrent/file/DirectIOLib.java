 ...
 * It also calculates Linux kernel version-dependent alignment amount (in bytes) for use with the <tt>O_DIRECT</tt>
 * flag,
...
public class DirectIOLib {
    ...
    static {
        ...
        try {
            if (!Platform.isLinux()) {
                ...
                /* From "man 2 open":
                 *
                 * O_DIRECT support was added under Linux in kernel version 2.4.10.  Older Linux kernels simply
                 * ignore this flag.  Some file systems may not implement
                ...
                if (binit) {
                    ...
                    logger.warn(String.format("O_DIRECT not supported on your version of Linux: %d.%d.%d", linuxVersion,
                            majorRev, minorRev));
                }
            }
        } catch (Throwable e) {
    ...
    public static DirectIOLib getLibForPath(String workingDir) {
        ...
        if (fsBlockSize == -1) {
            logger.warn("O_DIRECT support non available on your version of Linux (" + System.getProperty(
                    "os.version") + "), " +
    ...
    public static final boolean isBinit() {
        boolean tmp = DirectIOLib.binit;
        return tmp;
    }
